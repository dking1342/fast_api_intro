...
async def login(
    ...
    access_token = authentication.create_access_token(data={"user_id": str(user.user_id)})
    response = auth_schema.AuthResponse(
        status=status.HTTP_200_OK,
        message="Successful login"
    )
    response.data.append(auth_schema.UserLoginCreate(user_id=str(user.user_id), token=access_token))
